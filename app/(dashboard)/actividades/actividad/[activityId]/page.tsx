...
export default async function ActivityDetail({ params }: { params: { activityId: string } }) {
  ...
  const { data: a } = await s
    .from('activities')
    .select('id, artist_id, type, status, date, time, municipality, province, country, capacity, pay_kind, tags, venues(id,name,photo_url,address), group_companies(id,nick,name,logo_url)')
  ...
  const { data: promoterLink } = await s
    ...
    .eq('activity_id', a.id).maybeSingle()
  ...
  async function saveBasics(formData: FormData) {
    ...
    const { error } = await s.from('activities').update({ status, date, time, municipality, province, country, capacity, pay_kind }).eq('id', params.activityId)
    if (error) throw new Error(error.message)
    revalidatePath(`/actividades/${params.activityId}`)
  }
  ...
  async function attachPromoter(formData: FormData) {
    ...
    if (mode !== 'existing' && !counterparty_id) {
      ...
      const ins = await s.from('counterparties').insert({ organization_id: (await s.from('organizations').select('id').limit(1).single()).data!.id, legal_name, is_company, tax_id, as_third_party: true }).select('id').single()
  ...
  async function addAgent(formData: FormData) {
    ...
    if (mode !== 'existing' && !counterparty_id) {
      ...
      const is_company = String(formData.get('kind')||'') === 'company'
      if (!legal_name) throw new Error('Nombre requerido')
      const ins = await s.from('counterparties').insert({ organization_id: (await s.from('organizations').select('id').limit(1).single()).data!.id, legal_name, is_company, as_third_party: true }).select('id').single()
      if (ins.error) throw new Error(ins.error.message)
      counterparty_id = ins.data.id
    }
  ...
  async function saveEquipment(formData: FormData) {
    ...
    if (rider && rider.size>0) {
      ...
      const up = await s.storage.from('contracts').upload(`activities/${params.activityId}/rider_${crypto.randomUUID()}.pdf`, rider, { cacheControl: '3600', upsert: false, contentType: 'application/pdf' })
  ...
  async function addLocalProduction(formData: FormData) {
    ...
    if (error) throw new Error(error.message)
    // solicitud de factura (recibida) automática
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/actividades/${a.artist_id}`} className="btn-secondary">Volver</Link>
      </div>
        ...
        <form action={saveBasics} className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <form action={setCompany}>
              <select name="company_id" defaultValue={a.group_companies?.id || ''} className="w-full border rounded px-3 py-2">
                <option value="">(sin empresa)</option>
                {/* server-side: listado inline */}
                {/* eslint-disable-next-line react/jsx-key */}
                {a.group_companies ? <option value={a.group_companies.id}>{companyLabel(a.group_companies)}</option> : null}
              </select>
      ...
      <ModuleCard title="Promotor" leftActions={<span className="badge">Editar</span>}>
        ...
        {promoterLink?.counterparties?.id && (
          <div className="mt-3 flex items-center gap-3">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={promoterLink.counterparties.logo_url || '/avatar.png'} className="w-8 h-8 rounded-full border object-cover" alt="" />
            <Link href={`/terceros/${promoterLink.counterparties.id}`} className="underline font-medium">
              {promoterLink.counterparties.nick || promoterLink.counterparties.legal_name}
            </Link>
          </div>
        )}
      </ModuleCard>
        ...
        <div className="mt-3 text-sm">
          {(agents || []).map(a => (
            <div key={a.id} className="py-1">
              {a.counterparties?.nick || a.counterparties?.legal_name} · {a.commission_type === 'fixed' ? `${a.commission_amount?.toLocaleString('es-ES',{style:'currency',currency:'EUR'})}` : `${a.commission_pct}% (${a.commission_base})`}
            </div>
          ))}
