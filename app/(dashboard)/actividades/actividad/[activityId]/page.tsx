...
export default async function ActivityPage({ params }: { params: { id: string } }) {
  ...
  const { data: companies } = await s.from('group_companies').select('id, nick, name, logo_url').order('name', { ascending: true })

  // SERVER ACTION: guardar datos básicos
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <img src={a.artists?.avatar_url || '/avatar.png'} className="h-10 w-10 rounded-full border object-cover" alt="" />
        <div>
          <div className="text-xl font-semibold">{a.artists?.stage_name}</div>
          <div className="text-sm text-gray-600">{a.type || 'Actividad'} · {a.date || '—'}</div>
        </div>
      </div>

      {/* Barra para editar toda la pestaña */}
      <TabEditToolbar />
          ...
          childrenView={
            <div className="grid md:grid-cols-3 gap-3 text-sm">
              ...
              <div className="md:col-span-3">
                <span className="text-gray-500">Lugar:</span> {[a.municipality, a.province, a.country].filter(Boolean).join(', ') || '—'}
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {a.company?.logo_url && <img src={a.company.logo_url} className="h-5 w-auto object-contain border rounded bg-white" alt="" />}
          ...
          childrenEdit={
            <div className="grid md:grid-cols-3 gap-3">
              <div>
                ...
                <select name="status" defaultValue={a.status || 'draft'} className="border rounded px-3 py-2 w-full">
              ...
              <div>
                ...
                <select name="type" defaultValue={a.type || 'concert'} className="border rounded px-3 py-2 w-full">
              ...
              <div>
                ...
                <input type="date" name="date" defaultValue={a.date || ''} className="border rounded px-3 py-2 w-full" />
              </div>
              <div>
              ...
              <div>
                ...
                <input name="municipality" defaultValue={a.municipality || ''} className="border rounded px-3 py-2 w-full" />
              </div>
              <div>
                <div className="text-sm mb-1">Provincia</div>
                <input name="province" defaultValue={a.province || ''} className="border rounded px-3 py-2 w-full" />
              </div>
              <div>
                <div className="text-sm mb-1">País</div>
                <input name="country" defaultValue={a.country || 'España'} className="border rounded px-3 py-2 w-full" />
              </div>
              <div>
                <div className="text-sm mb-1">Aforo</div>
                <input type="number" name="capacity" defaultValue={a.capacity ?? ''} className="border rounded px-3 py-2 w-full" />
              </div>
              <div>
                <div className="text-sm mb-1">Pago</div>
                <select name="pay_kind" defaultValue={a.pay_kind || 'pay'} className="border rounded px-3 py-2 w-full">
              ...
              <div className="md:col-span-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {a.company?.logo_url && <img src={a.company.logo_url} className="h-6 w-auto object-contain border rounded bg-white" alt="" />}
                </div>
              </div>
            </div>
          }
      ...
      {/* Aquí podrás replicar el patrón con más módulos/pestañas reutilizando <ViewEditModule /> */}
    </div>
  )
}
