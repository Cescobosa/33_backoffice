...
export default async function ThirdParties({ searchParams }: { searchParams: { q?: string; archived?: string } }) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link className="btn-secondary" href={archived?'/terceros':'/terceros?archived=1'}>{archived?'Ver activos':'Archivados'}</Link>
          <Link className="btn" href="/terceros/new">+ Nuevo tercero</Link>
        </div>
      </div>
      <ClientSearch placeholder="Buscar terceros..." />
      <div className="divide-y divide-gray-200">
        {data?.map(t => (
          <Link key={t.id} href={`/terceros/${t.id}`} className="flex items-center gap-3 py-3 hover:bg-gray-50 rounded-md px-2">
