...
/**
 * En algunas consultas Supabase devuelve la relación como array; en otras como objeto.
 * Esta función lo normaliza para usarlo de forma segura.
 */
...
export default async function ThirdDetail({ params }: { params: { id: string } }) {
  ...
  // Artistas vinculados a este tercero
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-gray-600">
            {cp.is_company ? 'Empresa' : 'Particular'}
          </div>
        </div>
        <Link className="btn-secondary" href="/terceros">
          Volver
        </Link>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-sm text-gray-500">
          Ficha del tercero (vista mínima; sin 404).
        </div>
      </div>
      ...
      <div className="border rounded p-3">
        ...
        <div className="divide-y divide-gray-200">
          {!links?.length && (
            <div className="text-sm text-gray-500">Sin vínculos.</div>
          )}

          {links?.map((l: any) => {
            const a = artistFromLink(l) // <- normalización segura
            ...
            return (
              ...
              >
                ...
                <div className="font-medium">
                  {a?.stage_name || 'Artista'}
                </div>
                {l.status === 'unlinked' && (
                  <span className="badge badge-red ml-auto">Desvinculado</span>
                )}
              </Link>
            )
          })}
