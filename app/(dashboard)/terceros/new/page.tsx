...
export default function NewThird() {
  async function createThird(formData: FormData) {
    ...
    const { data: created, error } = await s.from('counterparties')
      .upsert({ organization_id: org, legal_name, is_company: kind, tax_id, as_third_party: true }, { onConflict: 'unique_key' })
      .select('id, organization_id').single()
    if (error) throw new Error(error.message)
    await s.from('counterparties').update({ as_third_party: true, nick }).eq('id', created.id)
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={createThird} className="space-y-4 max-w-xl">
        ...
        <div className="flex gap-3"><Link href="/terceros" className="btn-secondary">Cancelar</Link><button className="btn">Crear</button></div>
      </form>
    </div>
  )
}
