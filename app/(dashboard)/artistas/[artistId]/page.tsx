...
import ArtistThirdPartiesBlock from '@/components/ArtistThirdPartiesBlock'
...
async function getMinRules(artistId: string) {
  const s = createSupabaseServer()
  const { data, error } = await s
    .from('min_exempt_rules')
    .select(
      'id, income_type_id, rule_kind, until_amount_total, until_op_count, until_date, until_artist_generated_amount, base, active, income_types(id,name)',
    )
    .eq('artist_id', artistId)
    .order('created_at')
  if (error) throw new Error(error.message)
...
async function getAdvances(artistId: string) {
  ...
  const { data, error } = await s
    ...
    .select('id, income_type_id, amount, advance_date, note, income_types(id,name)')
    .eq('artist_id', artistId)
...
export default async function ArtistDetail({
  ...
  const [people, incomeTypes, configs, contracts, minRules, advances] = await Promise.all([
    getPeople(id),
  ...
  async function addPerson(formData: FormData) {
    ...
    const { error } = await s
      .from('artist_people')
      .insert({ artist_id: artist.id, role, full_name, dni, birth_date, phone, address })
    if (error) throw new Error(error.message)
    revalidatePath(`/artistas/${artist.id}`)
  ...
  async function addContract(formData: FormData) {
    ...
    const up = await s.storage.from('contracts').upload(path, pdf, {
      cacheControl: '3600',
      upsert: false,
  ...
  async function addShare(formData: FormData) {
    ...
    const { error: delErr } = await s
      .from('artist_group_shares')
      .delete()
      .eq('artist_id', artist.id)
      .eq('income_type_id', income_type_id)
    if (delErr) throw new Error(delErr.message)
    const rows = entries
      .filter((e) => e.pct > 0)
      .map((e) => ({ artist_id: artist.id, income_type_id, artist_person_id: e.pid, percentage: e.pct }))
    if (rows.length) {
  ...
  async function addMinRule(formData: FormData) {
    ...
    const until_artist_generated_amount = formData.get('until_artist_generated_amount')
      ? Number(formData.get('until_artist_generated_amount'))
      : null
  ...
  async function saveFiscal(formData: FormData) {
    ...
    if (cert && cert.size > 0) {
      ...
      const up = await s.storage.from('contracts').upload(path, cert, {
        cacheControl: '3600',
        upsert: false,
        contentType: cert.type || 'application/pdf',
      })
      if (up.error) throw new Error(up.error.message)
      const pub = s.storage.from('contracts').getPublicUrl(up.data.path)
    ...
    if (existing.data) {
      ...
      if (!fiscal_name || !tax_id || !fiscal_address || !iban) {
        revalidatePath(`/artistas/${artist.id}`)
        redirect(`/artistas/${artist.id}?tab=datos&sub=fiscales&mode=edit`)
        return
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {!isEdit ? (
            <Link className="btn" href={{ pathname: `/artistas/${artist.id}`, query: { tab: parentTab, sub, mode: 'edit' } }}>
              Editar ficha
            </Link>
          ) : (
            <Link className="btn-secondary" href={{ pathname: `/artistas/${artist.id}`, query: { tab: parentTab, sub } }}>
              Terminar edición
            </Link>
          )}
          {artist.status === 'active' ? (
            <form action={archiveArtist} method="post">
              <button className="btn-secondary">Archivar</button>
            </form>
          ) : (
            <>
              <form action={recoverArtist} method="post">
                <button className="btn-secondary">Recuperar</button>
              </form>
      ...
      {parentTab === 'datos' && (
        <>
              ...
              {!isEdit ? (
                ...
                <form action={updateBasic} method="post" className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    {artist.avatar_url && (
                      <img src={artist.avatar_url} alt="" className="w-24 h-24 rounded-full object-cover border mt-2" />
                    )}
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <label className="mr-4">
                      <input type="radio" name="artist_contract_type" value="general" defaultChecked={artist.artist_contract_type !== 'booking'} />{' '}
                      General
                    </label>
                    <label className="ml-4">
                      <input type="radio" name="artist_contract_type" value="booking" defaultChecked={artist.artist_contract_type === 'booking'} />{' '}
                      Booking
                    </label>
                  </div>
          ...
          {/* Módulo Promotor del artista */}
          {sub === 'basicos' && <ArtistPromoterModule artistId={params.artistId} />}
                ...
                {isEdit && (
                  <form action={addPerson} method="post" className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input name="dni" className="w-full border rounded px-3 py-2" />
                    </div>
                    <div>
                      <label className="block text-sm mb-1">Fecha de nacimiento</label>
                      <input type="date" name="birth_date" className="w-full border rounded px-3 py-2" />
                    </div>
                    <div>
                    ...
                    <div className="md:col-span-3">
                      <button className="btn">+ Añadir persona</button>
                    </div>
                  </form>
                )}
              ...
              {isEdit && (
                <form action={addContract} method="post" className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="date" name="signed_at" required className="w-full border rounded px-3 py-2" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Vencimiento</label>
                    <input type="date" name="expires_at" className="w-full border rounded px-3 py-2" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Renovación</label>
                    <input type="date" name="renew_at" className="w-full border rounded px-3 py-2" />
                  </div>
                  ...
                  <div className="md:col-span-2">
                    <button className="btn">+ Añadir contrato</button>
                  </div>
                </form>
              )}
                ...
                {contracts.map((c) => (
                  <div key={c.id} className="py-3">
                    ...
                    <div className="text-xs text-gray-600 mt-1">
                      {c.signed_at ? `Fecha firma: ${new Date(c.signed_at).toLocaleDateString()}` : ''}
                    </div>
                    <div className="mt-2">
                      <a href={c.pdf_url} target="_blank" className="underline text-blue-700">
                        Ver PDF
                      </a>
                    </div>
                  </div>
          ...
          {sub === 'condiciones' && (
            <>
                  ...
                  {isEdit ? (
                    configs.map((cfg) => (
                      <form key={cfg.id} action={addShare} method="post" className="border border-gray-200 rounded p-3 mb-4">
                        ...
                        <div className="mt-3">
                          <button className="btn">Guardar reparto</button>
                        </div>
                      </form>
                    ))
                  ) : (
                ...
                {isEdit && (
                  <form action={addMinRule} method="post" className="grid grid-cols-1 lg:grid-cols-4 gap-3">
                    ...
                    <div className="lg:col-span-4 grid grid-cols-1 md:grid-cols-4 gap-2">
                      ...
                      <input
                        name="until_artist_generated_amount"
                        type="number"
                        step="0.01"
                        placeholder="Generado por artista (opcional)"
                        className="border rounded px-2 py-1"
                      />
                    </div>
                    <div className="lg:col-span-4">
                      <button className="btn">+ Añadir regla</button>
                    </div>
                  </form>
                )}
                ...
                {isEdit && (
                  <form action={addAdvance} method="post" className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <div>
                      ...
                      <input name="advance_date" type="date" className="w-full border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm mb-1">Nota</label>
                      <input name="note" className="w-full border rounded px-2 py-1" />
                    </div>
                    <div className="md:col-span-4">
                      <button className="btn">+ Añadir adelanto</button>
                    </div>
                  </form>
                )}
                <div className="divide-y divide-gray-200 mt-4">
                  {advances.map((a) => (
                    <div key={a.id} className="py-2">
                      ...
                      <div className="text-sm text-gray-600">
                        ...
                        {Number(a.amount).toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
                      </div>
                    </div>
                  ))}
              ...
              {/* === TERCEROS VINCULADOS (NUEVO: componente unificado) === */}
              <ArtistThirdPartiesBlock
                artistId={artist.id}
                isEdit={isEdit}
                incomeTypes={incomeTypes}
                pathnameForRevalidate={`/artistas/${artist.id}?tab=datos&sub=condiciones${isEdit ? '&mode=edit' : ''}`}
              />
            </>
          )}
            ...
            <ModuleCard title="Datos fiscales" leftActions={<span className="badge">{isEdit ? 'Editar' : 'Ver'}</span>}>
              {isEdit && (
                <form action={saveFiscal} method="post" className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm mb-1">Factura como</label>
                  ...
                  <div>
                    <label className="block text-sm mb-1">Nombre fiscal</label>
                    <input name="fiscal_name" className="w-full border rounded px-2 py-1" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">DNI / CIF</label>
                    <input name="tax_id" className="w-full border rounded px-2 py-1" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Domicilio fiscal</label>
                    <input name="fiscal_address" className="w-full border rounded px-2 py-1" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">IBAN</label>
                    <input name="iban" className="w-full border rounded px-2 py-1" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Certificado titularidad (opcional)</label>
                    <input type="file" name="iban_cert" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Email liquidaciones</label>
                    <input name="settlement_email" className="w-full border rounded px-2 py-1" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Representante (nombre)</label>
                    <input name="agent_name" className="w-full border rounded px-2 py-1" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Representante (tel)</label>
                    <input name="agent_phone" className="w-full border rounded px-2 py-1" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Representante (email)</label>
                    <input name="agent_email" className="w-full border rounded px-2 py-1" />
                  </div>
                  <div className="md:col-span-2">
                    <button className="btn">Guardar fiscales</button>
                  </div>
                </form>
              )}
      ...
      {parentTab === 'actividades' && <ArtistActivitiesBlock artistId={artist.id} searchParams={searchParams} />}
...
async function ArtistActivitiesBlock({
  artistId,
  searchParams,
}: {
  artistId: string
  ...
  return (
    <ModuleCard
      ...
      leftActions={
        <div className="flex gap-2">
          <Link className="btn" href={`/actividades/new?artistId=${artistId}`}>
            + Nueva actividad
          </Link>
          <Link className="btn-secondary" href={`/actividades?artistId=${artistId}`}>
            Ver todas
          </Link>
        </div>
      }
    >
      <form className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-4" method="get">
        ...
        <div className="md:col-span-5 flex items-center gap-2">
          ...
          {!past ? (
            <Link className="btn-secondary" href={{ pathname: `/artistas/${artistId}`, query: { ...searchParams, tab: 'actividades', past: '1' } }}>
              Ver pasadas
            </Link>
          ) : (
            <Link className="btn-secondary" href={{ pathname: `/artistas/${artistId}`, query: { ...searchParams, tab: 'actividades', past: undefined } }}>
              Ver futuras
            </Link>
          )}
        </div>
      ...
      <div className="divide-y divide-gray-200 mt-4">
        {items.map((a) => (
          <ActivityListItem key={a.id} a={a} showArtist={false} />
        ))}
