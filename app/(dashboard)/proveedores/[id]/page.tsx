...
export default async function ProviderDetail({ params }: { params: { id: string } }) {
  ...
  async function saveBasics(formData: FormData) {
    ...
    const { error } = await s.from('counterparties').update({ is_company, nick, legal_name, tax_id, as_provider: true, ...(logo_url ? { logo_url } : {}) }).eq('id', id)
    if (error) throw new Error(error.message)
  ...
  async function addContract(formData: FormData) {
    ...
    if (error) throw new Error(error.message)
    revalidatePath(`/proveedores/${id}?tab=contratos`)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-gray-500">{c.is_company ? 'Empresa' : 'Particular'}</div>
        </div>
